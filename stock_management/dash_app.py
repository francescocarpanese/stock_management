...
# Custom CSS styling
CUSTOM_STYLE = {"backgroundColor": "#f8f9fa", "fontFamily": "Arial, sans-serif"}
                    ...
                    [
                        dbc.Col(
                            [
                                ...
                                dbc.NavbarBrand(
                                    "Sistema de Gestão de Stock Hospitalar",
                                    className="ms-2",
        ...
        [
            dbc.CardBody(
                [
                    ...
                    dbc.Button(
                        [html.I(className="fas fa-edit me-2"), "Corrigir Medicamento"],
                        id="btn-correct-drug",
                        color="info",
                        size="lg",
                        style=BUTTON_STYLE,
                    ...
                    dbc.Button(
                        [html.I(className="fas fa-file-alt me-2"), "Gerar Relatório"],
                        id="btn-report",
                        color="primary",
                        size="lg",
                        style=BUTTON_STYLE,
...
def create_drugs_table():
    ...
    return dbc.Card([dbc.CardBody([html.Div(id="table-container")])], style=CARD_STYLE)
...
def create_new_drug_modal():
    ...
    return dbc.Modal(
        [
            dbc.ModalHeader(
                dbc.ModalTitle("Novo Medicamento", style={"fontSize": "20px"})
            ),
            dbc.ModalBody(
                [
                    dbc.Row(
...
def create_new_movement_modal():
    ...
    return dbc.Modal(
        [
            dbc.ModalHeader(
                dbc.ModalTitle("Novo Movimento", style={"fontSize": "20px"})
            ),
            dbc.ModalBody(
                [
                    # Drug info display
...
def create_report_modal():
    ...
    if first_day_this_month.month == 1:
        start_date = first_day_this_month.replace(
            year=first_day_this_month.year - 1, month=12
        )
    else:
    ...
    return dbc.Modal(
        [
            dbc.ModalHeader(
                dbc.ModalTitle("Gerar Relatório", style={"fontSize": "20px"})
            ),
            dbc.ModalBody(
                [
                    dbc.Row(
...
@callback(
    ...
    [State("drug-modal", "is_open"), State("selected-drug-id", "data")],
    prevent_initial_call=True,
)
...
def manage_drug_form(
    ...
    # Handle saving drug
        ...
        if success:
            alert = dbc.Alert(
                "Medicamento guardado com sucesso!", color="success", duration=3000
            )
            ...
            alert = dbc.Alert(
                "Erro ao guardar medicamento. Verifique os campos.",
                color="danger",
                duration=3000,
            )
            return (
...
@callback(
    ...
    [State("movement-modal", "is_open"), State("selected-drug-id", "data")],
    prevent_initial_call=True,
)
def toggle_movement_modal(n_new, n_close, newly_created_id, is_open, selected_id):
    ...
    trigger_id = ctx.triggered_id
    print(
        f"DEBUG toggle_movement_modal: trigger={trigger_id}, newly_created_id={newly_created_id}, selected_id={selected_id}"
    )
    ...
    if trigger_id == "newly-created-drug-id":
        if newly_created_id:
            ...
            print(
                f"  -> Setting selected_drug_id to {newly_created_id} and opening modal"
            )
...
def load_drug_info_for_movement(n_clicks, newly_created_id, selected_id):
    ...
    drug_info = html.Div(
        [
            ...
            html.P(
                [html.Strong("Peças por Caixa: "), str(drug.get("pieces_per_box", ""))]
            ),
...
@callback(
    ...
    [Input("movement-boxes", "value"), Input("movement-pieces", "value")],
    [State("selected-drug-id", "data"), State("newly-created-drug-id", "data")],
)
...
def save_movement(
    n_clicks, mov_date, origin, boxes, pieces, mov_type, drug_id, newly_created_id
):
    ...
    # Debug logging
    print(
        f"DEBUG save_movement called: n_clicks={n_clicks}, drug_id={drug_id}, newly_created_id={newly_created_id}"
    )
    print(
...
@callback(
    ...
    [Input("btn-report", "n_clicks"), Input("close-report-modal", "n_clicks")],
...
@callback(
    ...
    [State("report-start-date", "date"), State("report-end-date", "date")],
    prevent_initial_call=True,
)
def generate_report(n_clicks, start_date, end_date):
    ...
    try:
        ...
        success, error_msg, folder_path = report_utils.generate_reports(
            conn, start_date=start_date_obj, end_date=end_date_obj
        )
...
def main():
    """Main entry point"""
    parser = argparse.ArgumentParser(description="Run the stock management app")
