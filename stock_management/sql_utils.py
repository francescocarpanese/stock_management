...
def add_drug(
    ...
    c = conn.cursor()
    c.execute(
        "INSERT INTO drugs (name, dose, units, expiration, pieces_per_box, type, lote, current_stock) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
        (name, dose, units, expiration, pieces_per_box, drug_type, lote, stock),
    )
    c.close()
    conn.commit()
...
def update_drug(
    ...
    c = conn.cursor()
    c.execute(
        "UPDATE drugs SET  name=?, dose=?, units=?, expiration=?, pieces_per_box=?, type=?, lote=? WHERE id=?",
    ...
    if last_inventory_date != None:
        c.execute(
            "UPDATE drugs SET  last_inventory_date=? WHERE id=?",
            (last_inventory_date, drug_id),
        )
    c.close()
    conn.commit()
...
def update_movement(
    ...
    c = conn.cursor()
    c.execute(
        "UPDATE movements SET date_movement=?, destination_origin=?, pieces_moved=?, movement_type=?, signature=? WHERE id=?",
        (
            date_movement,
            destination_origin,
            pieces_moved,
...
def movement_row_to_dict(row, columns):
    ...
    row_dict["date_movement"] = datetime.strptime(
        row_dict["date_movement"], "%Y-%m-%d"
    ).date()
    row_dict["entry_datetime"] = datetime.strptime(
        row_dict["entry_datetime"], "%Y-%m-%d %H:%M:%S"
    )
    return row_dict
...
def drug_row_to_dict(row, columns):
    row_dict = dict(zip(columns, row))
    row_dict["expiration"] = datetime.strptime(
        row_dict["expiration"], "%Y-%m-%d"
    ).date()
    row_dict["last_inventory_date"] = datetime.strptime(
        row_dict["last_inventory_date"], "%Y-%m-%d"
    ).date()
    return row_dict
...
def get_all_drugs_df(conn):
    ...
    df["last_inventory_date"] = df["last_inventory_date"].apply(
        lambda x: datetime.strptime(x, "%Y-%m-%d").date()
    )
    df["drug_id"] = df["id"]
    return df
...
def get_all_movements_df(conn):
    ...
    df["date_movement"] = df["date_movement"].apply(
        lambda x: datetime.strptime(x, "%Y-%m-%d").date()
    )
    df["entry_datetime"] = df["entry_datetime"].apply(
