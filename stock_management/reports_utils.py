...
def add_cum_stock_df(df, groupby_cols):
    ...
    for group_name, group_df in grouped_df:
        ...
        df.loc[indices, "last_inventory_date"] = group_df.loc[
            indices, "last_inventory_date"
        ]
        df.loc[indices, "stock_after_movement"] = group_df.loc[
            indices, "stock_after_movement"
        ]
        df.loc[indices, "last_inventory_stock"] = group_df.loc[
            indices, "last_inventory_stock"
        ]

    return df


def save_txt_mov_group(
    df,
    folder_path,
...
def save_txt_agg_per_ID(
    ...
    with open(out_path, "w") as f:
        for index, row in df_drug.iterrows():
            ...
            df_consumption_drug_id = df_consumption_ID[
                df_consumption_ID["drug_id"] == index
            ]
...
def compute_stock_group(
    ...
    for group_name, group_df in df_cum_grouped:
        ...
        inventory_on_last_inventory_date = group_df.loc[
            (group_df["date_movement"] == latest_last_inventory_date)
        ...
        idx_latest_mov_date = group_df[
            group_df["date_movement"] == latest_mov_date
        ].index
        ...
        stock = group_df.loc[
            (group_df["date_movement"] == latest_mov_date)
            & (group_df["entry_datetime"] == max_entry_datetime),
            "stock_after_movement",
        ].min()
...
def compute_consumption_group(
    ...
    # Filter by date
    df_cum = df_cum[
        (df_cum["date_movement"] >= start_date) & (df_cum["date_movement"] <= end_date)
    ]
    ...
    df_cum["entry"] = df_cum.apply(
        lambda x: x["pieces_moved"] if x["movement_type"] == "entry" else 0, axis=1
    )
    df_cum["exit"] = df_cum.apply(
        lambda x: x["pieces_moved"] if x["movement_type"] == "exit" else 0, axis=1
    )
    df_cum["inventory"] = df_cum.apply(
        lambda x: x["pieces_moved"] if x["movement_type"] == "inventory" else 0, axis=1
    )
